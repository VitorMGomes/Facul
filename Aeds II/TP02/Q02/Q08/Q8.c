...
void shellSort(Character *wizard, int tam){ 
    int separacao = 1;
    do
    { 
    ...
    do{
        ...
        for(int grupo=0; grupo<separacao; grupo++){ 
            for(int i=separacao+grupo; i<tam; i+=separacao){ 
                ...
                while(j>=0 && tmp.eyeColour < wizard[j].eyeColour){
                    wizard[j+separacao] = wizard[j];
                    j-=separacao;
                ...
                while(j>=0 && tmp.eyeColour == wizard[j].eyeColour && strcmp(tmp.name, wizard[j].name)<0){
                    wizard[j+separacao] = wizard[j];
                    j-=separacao;
