...
// javac Q9.java; cat pub.in | java Q9 > saida.out -- "/tmp/characters.csv"
...
public class Q9 {
    ...
    public static int compareIntersection(int j, int i, TP tp)
    {
        int retorno = (wizard.get(j).getHairColour().compareTo(wizard.get(i).getHairColour()));
        
        if(retorno == 0)
        {
            retorno = wizard.get(j).getName().compareTo(wizard.get(i).getName());
        }
    ...
    public static void heapSort(TP tp, int n) {
    ...
    private static void reconstruir(int i, int n, TP tp) {
        int filho = i;
        int esquerda = 2 * i + 1;
        int direita  = 2 * i + 2;

        if(esquerda < n && compareIntersection(esquerda, filho, tp) > 0) 
        {
            filho = esquerda;
        } 
    
        if(direita < n && compareIntersection(direita, filho, tp) > 0) 
        {
            filho = direita;
        } 
