...
// javac Q13.java; cat pub.in | java Q13 > saida.out -- "/tmp/characters.csv"
...
public class Q13 {
    ...
    public static int compareIntersection(Character j, Character tmp, TP tp)
    {
        int retorno = (j.getActorName().compareTo(tmp.getActorName()));
        
        if(retorno == 0)
        {
            retorno = j.getName().compareTo(tmp.getName());
        }
    ...
    public static void mergeSort(int left, int right, TP tp){
        if (left < right) {
            int meio = (left + right) / 2;
    
            mergeSort(left, meio, tp); 
            mergeSort(meio + 1, right, tp); 
    
            merge(left, meio, right, tp);
        }
    }

    public static void merge(int left, int mid, int right, TP tp)
    {
        int n1 = mid - left + 1;
        int n2 = right - mid;

        Character[] auxLeft = new Character[n1];
        Character[] auxRight = new Character[n2];

        for(int i = 0; i < n1; i++)
        {
            auxLeft[i] = wizard.get(left + i);
            tp.addMov(1);
        }

        for(int i = 0; i < n2; i++)
        {
            auxRight[i] = wizard.get(mid + i + 1);
            tp.addMov(1);
        }
        ...
        while(i < n1 && j < n2)
        {
            if(compareIntersection(auxLeft[i], auxRight[j], tp) < 0)
            {
                wizard.set(l, auxLeft[i]);
                i++;
                tp.addMov(1);
            }
            else
            {
                wizard.set(l, auxRight[j]);
                j++;
                tp.addMov(1);
            }
    ...
    public static void main(String args[]) throws Exception {
        ...
        tp.ordenacaoFile("800643_mergesort.txt");
