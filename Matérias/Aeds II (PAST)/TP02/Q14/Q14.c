...
int compareIntersection(Character *wizard, int j, int i)
{
    int retorno = strcmp(wizard[j].hairColour, wizard[i].hairColour);

    if (retorno == 0)
    {
        retorno = strcmp(wizard[j].name, wizard[i].name);
    }
...
int findMAX(Character *wizard, int length) {
    int max = wizard[0].id;
...
void countingSortByDigit(Character *wizard, int length, int digit) {
    const int base = 10;
    int count[base];
    Character output[length];
    ...
    for (int i = length - 1; i >= 0; i--) {
        int digitValue = (*wizard[i].id / digit) % base;
        output[count[digitValue] - 1] = wizard[i];
        movimentacoes++;
        count[digitValue]--;
    }
...
void radixSort(Character *wizard, int length) {
    int max = findMAX(wizard, length);

    for (int digit = 1; max / digit > 0; digit *= 10) {
        countingSortByDigit(wizard, length, digit);
    }
}
