...
// javac Q16.java; cat pub.in | java Q16 > saida.out -- "/tmp/characters.csv"
...
public class Q16 {
    ...
    {
        int retorno = (wizard.get(j).getDateOfBirth().compareTo(tmp.getDateOfBirth()));
    ...
    public static void insertionSort(int len, TP tp)
    {
        int k = 10;
        for(int i = 1; i < len; i++)
        {
            Character tmp = wizard.get(i);
            tp.addMov(1);
            int j = (i < k) ? i - 1 : k - 1;

            while((j >= 0) && compareIntersection(j, tmp, tp) > 0)
            {
                wizard.set(j + 1, wizard.get(j));
                j--;
            }
    ...
    public static void main(String args[]) throws Exception {
        ...
        tp.ordenacaoFile("800643_insercaoParcial.txt");
