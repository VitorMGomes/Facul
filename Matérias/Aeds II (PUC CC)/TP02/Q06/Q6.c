...
bool pesquisaBinaria(char nome[], Character *wizard[], int tam)
{
    //puts("papas");
    ...
    while(left <= right)
    {
        ...
        int igual = strcmp(nome, wizard[meio]->name);
...
int loop(Character *wizard, int len, int j)
{
    if(len == j)
    {
        return j - 1;
    ...
    int min = j;
    int next = loop(wizard, len, j + 1);
    comparacoes++;
    if(strcmp(wizard[next].name, wizard[min].name) < 0 )
    {
        min = next;
    }
...
void selecaoRecursiva(int len, int i, Character *wizard)
{
    if(i == len)
    {
        return;
    ...
    int menor = loop(wizard, len, i + 1);
    
    comparacoes++;
    if(strcmp(wizard[i].name, wizard[menor].name) > 0 )
    {swap(wizard, i, menor);}
    

    selecaoRecursiva(len, i + 1, wizard);
...
int main()
{
    ...
    //     }
          ...
          //if(pesquisaBinaria(nome, &wizard, n))
          //{
          //    puts("SIM");
          //}
          //else{
