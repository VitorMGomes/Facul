...
// javac Q15.java; cat pub.in | java Q15 > saida.out -- "/tmp/characters.csv"
...
public class Q15 {
    ...
    public static void selectionSort(int len, TP tp)
    {
        for(int i = 0; i < 10; i++)
        {
            int menor = i;
            ...
            {
                tp.addComp(1);
                if(wizard.get(j).getName().compareTo(wizard.get(menor).getName()) < 0) // wizard.get(menor).getName().compareTo(wizard.get(j).getName()) > 0
                {
                    menor = j;
                }
            }
    ...
    public static void main(String args[]) throws Exception {
        ...
        tp.ordenacaoFile("800643_selecaoParcial.txt");
