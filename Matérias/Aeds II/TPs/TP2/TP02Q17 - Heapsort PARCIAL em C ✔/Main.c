...
void constroi(Pokemon *pokemons, int tamHeap, int i) {
    while (i > 1) {
        ...
        // Comparar a altura (height) dos Pokémons
...
void reconstroi(Pokemon *pokemons, int tamHeap) {
    ...
    while (i <= (tamHeap / 2)) {
        ...
        // Comparar altura entre o pai e o maior filho
...
void heapsortParcial(Pokemon *array, int n) {
    ...
    // Copiar para tmp começando no índice 1
    for (int i = 0; i < n; i++) {
        tmp[i + 1] = array[i];
    }

    int k = 10;  // Tamanho parcial
    ...
    // Copiar de volta os Pokémons ordenados para o array original
    for (int i = 0; i < n; i++) {
        array[i] = tmp[i + 1];
    }
...
int main()
{
    ...
    FILE *fw = fopen("800643_heapsortParcial.txt", "w");
    if (fw != NULL)
    {
