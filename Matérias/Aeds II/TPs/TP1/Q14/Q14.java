...
class Recursive{
    ...
    Recursive(String input){
        this.a = input.charAt(2) == '0' ? false : true;
        this.b = input.charAt(4) == '0' ? false : true;
        
        if(input.charAt(0) == '3'){
            this.c = input.charAt(6) == '0' ? false : true;

            if(input.charAt(input.length() - 1) == ' '){
                this.eq = Q14.substring(input, 8, input.length() - 1);
            }else{
                this.eq = Q14.substring(input, 8, input.length());
            }
        }else{
            if(input.charAt(input.length() - 1) == ' '){
                this.eq = Q14.substring(input, 6, input.length() - 1);
            }else{
                this.eq = Q14.substring(input, 6, input.length());
            }            
        }
...
public class Q14{
    ...
    public static String substring(String input, int start, int end){
        ...
        if(start < end){
            newStr = input.charAt(start) + substring(input, start + 1, end);
        }
    ...
    public static void main(String[] args){
        ...
        while(input.charAt(0) != '0'){
            ...
            Recursive equation = new Recursive(input);
